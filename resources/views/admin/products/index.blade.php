...
@section('content')
    <div class="mb-8 flex justify-between items-end">
        ...
        <a href="{{ route('admin.products.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm">
            + Add New Product
        </a>
    </div>
    ...
    @endif
    @if(session('success'))
        <div class="mb-4 bg-green-50 border border-green-200 text-green-600 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
                ...
                <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                    @forelse($products as $product)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                <span class="bg-gray-100 border border-gray-200 text-gray-600 px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-bold">
                            ...
                            <td class="py-4 px-6 text-center">
                                @if($product->stock < 10)
                                    <span class="bg-red-50 text-red-600 border border-red-100 px-3 py-1 rounded-full text-xs font-bold">{{ $product->stock }}</span>
                                @else
                                    <span class="bg-green-50 text-green-600 border border-green-100 px-3 py-1 rounded-full text-xs font-bold">{{ $product->stock }}</span>
                                @endif
                            </td>
                            ...
                            <td class="py-4 px-6">
                                ...
                                @else
                                    <div class="w-12 h-12 rounded-lg bg-gray-50 border border-gray-200 flex items-center justify-center text-gray-400 text-xs font-medium">
                                        No img
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="py-4 px-6">
                                <div class="flex items-center justify-end gap-4">
                                    <a href="{{ route('admin.products.edit', $product->id) }}" class="text-gray-400 hover:text-blue-600 transition" title="Edit">
